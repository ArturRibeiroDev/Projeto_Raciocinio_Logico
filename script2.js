...
let nota_prova_1= parseFloat(prompt('Qual a nota da AV1? '))
let nota_prova_2 = parseFloat(prompt('Qual a nota da AV2? '))
...
if (nota_parcial_aluno >= 4) {
    let nota_prova_3 = parseFloat(prompt('Qual a nota da AV3? '))
    let nota_final = (nota_parcial_aluno + nota_prova_3) / 2
    let percentual_frequencia = parseFloat(prompt('Qual o percentual de frequencia do aluno? '))
    if (percentual_frequencia < 75 && nota_final >= 5){
        console.log('Mesmo conseguindo uma nota final superior a 5, você foi reprovado por falta...')
    }
